...
void View::addDrawable(std::shared_ptr<IDrawable> elem) {
    objects.push_back(elem);
}

void View::removeDrawable(std::shared_ptr<IDrawable> elem) {
    auto last = std::remove(objects.begin(), objects.end(), elem);
